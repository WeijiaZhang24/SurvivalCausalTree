...
int
  USER3init(int n, double *y[], int maxcat, char **error,
...
void USER3ss(int n, double *y[], double *value,  double *con_mean, double *tr_mean,
...
void USER3(int n, double *y[], double *x, int nclass, int edge, double *improve, double *split,
    ...
    if (nclass == 0) {
      ...
      for (i = 0; right_n > edge; i++) {
        ...
        if (x[i + 1] != x[i] && left_n >= edge &&
            ...
            temp2 = left_effect + right_effect - node_effect; 		//CT
            //temp3 = left_effect2 + right_effect2 - node_effect2; 	//fit
      ...
      for (i=0; i < count; i++)
      {
        ...
        for (j=0; j < count; j++)
          {
          a[0] = criterion1[i]; a[1] = criterion2[i];
          b[0] = criterion1[j]; b[1] = criterion2[j];
          if (is_dominated(a,b,1) ==1 || criterion2[i] <= 0 || criterion1[i] <=0)
            num_dominated[i]++;
          }
      }
      int index;
      double criterion_best;
...
int is_dominated(double *n1, double *n2, int k){
//check whether a is pareto dominated by b
  int lte = 0; //count of a[k] <= b[k]
  int lt = 0; //count of a[k] < b[k]
  for (int i = 0; i < k; i++){
    if (n1[i] <= n2[i]){
      //printf("%f\n", (1 + epsilon));
      lte++;
...
double
  USER3pred(double *y, double wt, double treatment, double *yhat, double propensity)
  {
    double ystar;
    double temp;
