...
causalTree <- function(formula, data, weights, treatment, subset,
	...
	if (missing(propensity)) {
		propensity <- rep(sum(treatment) / nobs,nobs)
	}
  ## check complete case indicator
	if (missing(completeCase)){
	  completeCase <- rep(1,nobs)
	}
	...
	split.Rule.int <- pmatch(split.Rule, c("TOT", "CT", "fit", "tstats", "TOTD", "CTD",
	                                       "fitD", "tstatsD", "survival", "userD", "survival2",
	                                        "user3", "user4", "user5"))
	if (is.na(split.Rule.int)) stop("Invalid splitting rule.")
	split.Rule <- c("TOT", "CT", "fit", "tstats", "TOTD", "CTD", "fitD",
	                "tstatsD", "survival", "userD", "survival2", "user3", "user4")[split.Rule.int]
	...
	# check propensity score:
	if (split.Rule %in% c("TOT", "TOTD", "fit", "fitD", "CT", "survival", "CTD")) {
		if (any(propensity > 1) || any(propensity < 0)) {
	...
	if (cv.option == "CT" || cv.option == "fit" || cv.option == "user") {
		if (cv.Honest) {
			# cv.Honest = T
			cv.option <- paste(cv.option, 'H', sep = '')#include <R.h>
	...
	if (!is.null(xlevels))
		...
		storage.mode(propensity) <- "double"
    ...
    if (split.Rule.int == 9){
      ...
      censoringProb <- rep(1,length(treatment))
    }
